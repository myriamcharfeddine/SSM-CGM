"""
mamba288_static_local.py
=========================
Experiment B — dynamic + static features, original split config.

What differs from Experiment A (mamba288_local.py):
  - Reads enriched feathers from Data/ssmcgm_ready_enriched/
  - Loads static_feature_list.json to know which extra columns to pass
    as static_reals to TimeSeriesDataSet
  - create_tft_dataloaders_static() adds those columns to the TFT's
    static_variable_selection VSN (no architecture change required)
  - Outputs go to Data/results/exp_B_smoke/ or Data/results/exp_B_full/
  - Results saved to results_dynamic_static.csv

Static covariates added (see static_feature_list.json):
  Numeric baselines: hba1c_percent, bmi, waist_to_hip_ratio, BP, HR,
                     serum_glucose, c_peptide, HDL, LDL, triglycerides
  Binary medication flags: metformin, insulin, GLP-1, SGLT2, sulfonylurea,
                            thiazolidinedione, any_diabetes_drug
  One-hot sex: demo_sex_at_birth_{female,male,unknown,...}

Why late fusion via TFT static branch:
  TFT's static_variable_selection VSN learns weighted static embeddings
  that condition the encoder and enrich the decoder — this IS late fusion.
  No custom architecture needed; interpretability (VSN weights) is free.

Run:
  # Smoke test (200 participants, 3 epochs)
  python mamba288_static_local.py --smoke

  # Full run
  python mamba288_static_local.py

  # Eval only
  python mamba288_static_local.py --eval-only
"""
...
# =============================
# Mamba blocks (identical to mamba288_local.py / Experiment A)
# =============================
...
# =============================
# MambaTFT (identical to Experiment A)
# =============================
...
# =============================
# Data — extended with extra static_reals
# =============================
...
def create_tft_dataloaders_static(train_df, param, extra_static_reals=None, max_val_windows=None):
    """
    Same as Exp A dataloaders, but with extra_static_reals appended to
    the TimeSeriesDataSet's static_reals list.  MambaTFT.from_dataset()
    automatically sizes its static VSN to accommodate them.
    """
    log_memory("Building TimeSeriesDataSets (static)")
...
# =============================
def validate_before_training(train_df, extra_static_reals):
    print("\n[Pre-training Validation]")
    missing = [c for c in extra_static_reals if c not in train_df.columns]
    if missing:
        raise ValueError(f"Static columns missing from train DataFrame: {missing}")

    nan_counts = train_df[extra_static_reals].isna().sum()
    bad = nan_counts[nan_counts > 0]
    if len(bad) > 0:
        raise ValueError(f"NaN found in static columns:\n{bad}")

    inf_mask = np.isinf(
        train_df[extra_static_reals].select_dtypes(include=[float, int]).values
    ).any()
    if inf_mask:
        raise ValueError("Inf values found in static columns")

    n_pids = train_df["participant_id"].nunique()
    print(f"  Participants     : {n_pids:,}")
    print(f"  Total rows       : {len(train_df):,}")
    print(f"  Dynamic features : 29 (Exp A schema)")
    print(f"  Static features  : {1 + len(extra_static_reals)} (age + {len(extra_static_reals)} new)")
    print(f"  Static cols OK   : no NaN, no Inf")
...
def TFT_train_static(train_df, param, out_dir: Path, extra_static_reals=None,
                     limit_train_batches=None, save_all_checkpoints=False, max_val_windows=None):
    training, val_dl, train_dl, validation = create_tft_dataloaders_static(
        train_df, param, extra_static_reals=extra_static_reals, max_val_windows=max_val_windows
    )

    # Print first-batch shapes for diagnostics
    try:
        sample_x, sample_y = next(iter(train_dl))
        print(f"\n[Shapes] encoder_cont : {sample_x['encoder_cont'].shape}")
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename="mamba288-static-{epoch:02d}-{val_loss:.2f}",
...
def evaluate(tft, val_dl, out_dir: Path, ctx_h: int = 24):
    ...
    results = pd.DataFrame([{
        "model":        f"SSM-CGM-static (mamba288, {ctx_h}h ctx, 1h horizon)",
        "context_h":    ctx_h,
        "horizon_h":    1,
        "n_windows":    n_windows,
        "rmse":         rmse,
    ...
    results_path = out_dir / "results_dynamic_static.csv"
...
def subsample_participants(df, n, seed=42):
    ...
    for g in groups:
        ...
        sampled.extend(np.random.default_rng(seed).choice(pool, size=k, replace=False).tolist())
...
def parse_args():
    p = argparse.ArgumentParser(description="SSM-CGM mamba288-static (Experiment B)")
    p.add_argument("--train", type=Path,
                   default=Path("/home/myriamcharfeddine/CGM/Data/ssmcgm_ready_enriched/train_timeseries_static.feather"))
    p.add_argument("--test",  type=Path,
                   default=Path("/home/myriamcharfeddine/CGM/Data/ssmcgm_ready_enriched/test_timeseries_static.feather"))
    p.add_argument("--out",   type=Path,
                   default=Path("/home/myriamcharfeddine/CGM/Data/results/exp_B_full"))
    p.add_argument("--static-feature-list", type=Path,
                   default=Path("/home/myriamcharfeddine/CGM/Data/ssmcgm_ready_enriched/static_feature_list.json"))
    p.add_argument("--eval-only", action="store_true",
                   help="Skip training; load best checkpoint and evaluate")
    p.add_argument("--smoke", action="store_true",
                   help="Smoke test: 200 stratified participants, 3 epochs")
...
if __name__ == "__main__":
    ...
    for fpath, label in [(args.train, "train feather"), (args.test, "test feather"),
                         (args.static_feature_list, "static_feature_list.json")]:
        if not fpath.exists():
            raise FileNotFoundError(
                f"[ERROR] {label} not found: {fpath}\n"
                f"  Run prepare_ssmcgm_data_enriched.py first."
            )
    ...
    if args.smoke:
        ...
        args.out = Path("/home/myriamcharfeddine/CGM/Data/results/exp_B_smoke")
    ...
    print(f"\n{'='*56}")
    print(f"  Experiment B — SSM-CGM Dynamic+Static (Mamba288)")
    print(f"{'='*56}")
    ...
    # ── Pre-training validation ───────────────────────────────────────────────
    validate_before_training(train_df, extra_static_reals)

    if args.eval_only:
        ckpt_dir = args.out / "checkpoints"
        ckpts = sorted(ckpt_dir.glob("mamba288-static-*.ckpt"))
        if not ckpts:
            raise FileNotFoundError(f"No checkpoint in {ckpt_dir}")
        best_ckpt = ckpts[-1]
        print(f"Loading checkpoint: {best_ckpt}")
        _, val_dl, _, _ = create_tft_dataloaders_static(
            train_df, param, extra_static_reals, max_val_windows=args.max_val_windows
        )
        tft = MambaTFT.load_from_checkpoint(str(best_ckpt), weights_only=False)
        evaluate(tft, val_dl, args.out, ctx_h=ctx_h)
    else:
        tft, trainer, val_dl, train_dl, validation, training = TFT_train_static(
            train_df, param, args.out, extra_static_reals=extra_static_reals,
