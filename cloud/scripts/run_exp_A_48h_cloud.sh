#!/usr/bin/env bash
# run_exp_A_48h_cloud.sh
# ======================
# Cloud Batch runner for Experiment A — dynamic only, 48h (576-bin) context.
# Runs inside the gpu-base container on a2-highgpu-4g (4×A100).
#
# Flow:
#   1. Start nvidia-smi logger
#   2. Download feathers from GCS → /tmp/exp_A_48h_data/
#   3. Run mamba288_local.py with --context-length 576 (30 epochs, 4 GPUs)
#   4. Upload checkpoint + results to GCS on exit (via trap)

set -euo pipefail

GCS_SCRIPTS="gs://cgmproject2025/scripts/SSMCGM"
GCS_DATA="${GCS_SCRIPTS}/data_exp_A"
GCS_OUT="gs://cgmproject2025/results/exp_A_48h"
PYTHON="/opt/micromamba/envs/cgmenv/bin/python"
SCRIPTS_DIR="/mnt/disks/scripts"
DATA_DIR="/tmp/exp_A_48h_data"
RESULTS_DIR="/tmp/exp_A_48h_results"
...
gsutil -m cp "${GCS_DATA}/train_timeseries.feather" "${DATA_DIR}/"
gsutil -m cp "${GCS_DATA}/test_timeseries.feather"  "${DATA_DIR}/"
...
"${PYTHON}" "${SCRIPTS_DIR}/mamba288_local.py" \
    --train  "${DATA_DIR}/train_timeseries.feather" \
    --test   "${DATA_DIR}/test_timeseries.feather"  \
    --out    "${RESULTS_DIR}" \
    --epochs 30 \
    --context-length 576 \
    > "${TRAIN_LOG}" 2>&1
