...
% -----------------------------------------------------------------------------
\section{Interpretability: Two Views of Model Reliance}
\label{sec:interp}

\subsection{Question and Scope}
\label{subsec:interp_scope}

This section asks a narrow question: \emph{what does the trained AI-READI
forecasting model rely on?}
The answer is reported through two complementary views of the same frozen base
model.
...
These views answer different scientific questions and should not be collapsed
into one ranking.

All results in this section use the full held-out AI-READI test split:
153{,}591 dense anchors sampled at a 15-minute stride across 381
participant/segment streams from 221 test participants.
The analysis is not restricted by insulin-status metadata.
The variable \texttt{med\_insulin} is static therapy metadata, not an observed
insulin action, so no insulin-on-board or dose-response interpretation is made.
The meal-proxy channel is retained only as a diagnostic input group; AI-READI
has no confirmed meal timestamps and no carbohydrate quantities.

\subsection{The Two Reliance Views}
\label{subsec:interp_two_views}

\paragraph{View 1: output-level reliance.}
Leave-one-modality-out (LOMO) ablation measures the mean absolute change in
the final 12-step forecast when one modality is replaced by its population
mean.
This view is defined for all 11 modality groups because every group eventually
...
\paragraph{View 2: state-construction reliance.}
The rigorous joint-group norm measures how much a modality contributes to the
...
Summing before taking the norm matters because many channels are collinear:
heart-rate mean/min/max/std and multiple activity-stage indicators can point in
similar directions in representation space.
A post-hoc sum of individual norms would double-count those shared directions.

The $h_t$ view is available only for the 6 groups that pass through the
...
The other 5 groups are not missing from the analysis; they enter the model by
different architectural paths.
...
Those groups are therefore interpreted through output-level LOMO, not through
an $h_t$ contribution vector.
...
\input{sections/local_case_studies_revision}
